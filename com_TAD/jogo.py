...
class forca:
  ...
  def mostrar_estado(self):
    ...
    personagem = self.personagem.get_elemento()
    ...
    tentadas = self.tentadas.converter_para_string()
  ...
  def iniciar(self):
    self.chance = 6
    self.tentadas = Lista_ordenada()
    self.palavra = self.escolher_proxima_palavra()
    self.personagem = Lista(text.ascii_personagem)
  ...
  def jogar(self, letra): 
    ...
    if letra >= 'a' and letra <= 'z':
      ...
      elif not self.tentadas.busca(letra):
        self.tentadas.inserir(letra)
        self.personagem.remover()
        self.chance -= 1
